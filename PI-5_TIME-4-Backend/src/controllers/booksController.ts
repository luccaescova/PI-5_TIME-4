...
export async function recommendBooksController(req: Request, res: Response) {
  try {
    ...
    if (!tags || !Array.isArray(tags) || tags.length === 0) {
      return res.status(400).json({ message: 'Informe pelo menos uma tag' });
    }

    const pyUrl = process.env.RECOMMENDATION_SERVICE_URL || 'http://localhost:5000';

    const response = await fetch(`${pyUrl}/recommend`, {
      method: 'POST',
    ...
    if (!response.ok) {
      return res.status(502).json({ message: 'Erro no serviço de recomendação' });
    }
    ...
    console.error(err);
    return res.status(503).json({ message: 'Serviço de recomendação indisponível. Certifique-se de que o serviço Python está rodando.' });
  }
}
