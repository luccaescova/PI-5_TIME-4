...
export default async function seedDatabase() {
  ...
  const tagsOz = ['fantasia', 'aventura', 'clássico', 'domínio público', 'infantil', 'família'];
  const existingBook = await Book.findById(bookId);
  if (!existingBook) {
    await Book.create({ _id: bookId, titulo: 'O Mágico de Oz', autor: 'L. Frank Baum', tags: tagsOz });
    console.log('✓ livro seed criado:', bookId);
  } else {
    if (!existingBook.tags || existingBook.tags.length === 0) {
      await Book.updateOne({ _id: bookId }, { tags: tagsOz });
    }
  ...
  const tagsVidas = ['romance', 'leitura obrigatória', 'realismo', 'brasileiro', 'drama', 'histórico', 'sertão'];
  ...
  if (!existingBook2) {
    ...
    if (!existingBook2.tags || existingBook2.tags.length === 0) {
      await Book.updateOne({ _id: bookId2 }, { tags: tagsVidas });
    }
  ...
  const tagsDom = ['romance', 'leitura obrigatória', 'clássico', 'brasileiro', 'drama', 'ciúme', 'realismo'];
  ...
  if (!existingBook3) {
    ...
    if (!existingBook3.tags || existingBook3.tags.length === 0) {
      await Book.updateOne({ _id: bookId3 }, { tags: tagsDom });
    }
